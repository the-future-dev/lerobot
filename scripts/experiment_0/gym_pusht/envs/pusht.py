    ...
    * TODO:
    ...
    def _initialize_observation_space(self):
        if self.obs_type == "state":
            ...
            #         dtype=np.float64,
            #     )
        elif self.obs_type == "environment_state_agent_pos_privileged":
            self.observation_space = spaces.Dict(
                {
                    "environment_state": spaces.Box(
                        low=np.zeros(32),
        ...
        elif self.obs_type == "environment_state_diff":
            self.observation_space = spaces.Dict(
                {
                    "environment_state": spaces.Box(
                        low=np.zeros(16),
        ...
        elif self.obs_type == "pixels_privileged":
            self.observation_space = spaces.Dict({
                "pixels": spaces.Box(
                        low=0,
                        high=255,
            ...
            raise ValueError(
                ...
                "pixels_agent_pos, environment_state_agent_pos_privileged, environment_state_diff]"
            )
    ...
    def get_obs(self):
        ...
        if self.obs_type == "environment_state_agent_pos":
            ...
            return obs
        if self.obs_type == "environment_state_agent_pos_privileged":
            obs = {
                "environment_state": np.concatenate([
                    self.get_keypoints(self._block_shapes).flatten(),
                    self.get_keypoints(self._goal_shapes).flatten()
                ]),
                "agent_pos": np.array(self.agent.position),
            }
            return obs
        if self.obs_type == "environment_state_diff":
            obs = {
                "environment_state": self.get_keypoints(self._block_shapes).flatten()-self.get_keypoints(self._goal_shapes).flatten(),
                "agent_pos": np.array(self.agent.position),
            }
            return obs
        ...
        if self.obs_type == "pixels":
            ...
            return {
                ...
                "agent_pos": np.array(self.agent.position),
                "environment_state": self.get_keypoints(self._block_shapes).flatten(),
                "goal_state": self.get_keypoints(self._goal_shapes).flatten()
            }
