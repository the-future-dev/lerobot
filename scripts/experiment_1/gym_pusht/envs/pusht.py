...
class PushTEnv(gym.Env):
    ...
    * TODO:
    ...
    def _initialize_observation_space(self):
        if self.obs_type == "state":
        ...
        elif self.obs_type == "environment_diff_state":
            self.observation_space = spaces.Dict(
                {
                    "environment_state": spaces.Box(
                        low=np.zeros(16),
            ...
            raise ValueError(
                ...
                "pixels_agent_pos, environment_state_agent_pos_privileged, environment_diff_state]"
            )
        ...
        if self.obs_type == "environment_state_agent_pos_privileged":
            obs = {
                "environment_state": np.concatenate([
                    ...
                    self.get_keypoints(self._block_shapes).flatten()-self.get_keypoints(self._goal_shapes).flatten()
                ]),
                "agent_pos": np.array(self.agent.position),
            }
            return obs
        
        if self.obs_type == "environment_diff_state":
            obs = {
                "environment_state": np.concatenate([
                    self.get_keypoints(self._block_shapes).flatten()-self.get_keypoints(self._goal_shapes).flatten()
                ]),
